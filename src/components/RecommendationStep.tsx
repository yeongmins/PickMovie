...
import { discoverMovies, getPosterUrl, calculateMatchScore, GENRE_IDS, LANGUAGE_CODES, type TMDBMovie } from '../utils/tmdb';
...
export function RecommendationStep({ preferences, onComplete, onRestart, initialFavorites }: RecommendationStepProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 relative bg-[#1a1a24]">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-2">TMDB에서 최적의 영화를 검색하고 있습니다</p>
        </div>
      </div>
    );
  }
  ...
  if (movies.length === 0) {
    ...
    if (preferences.country !== '상관없음' && preferences.country !== '' && preferences.releaseYear && preferences.releaseYear !== '상관없음') {
      ...
      <div className="max-w-5xl mx-auto relative z-10 w-full">
        ...
        <div className="text-center mb-10 pt-8">
          ...
          <p className="text-gray-300 text-sm">
            {preferences.genres.length}개 장르 · {preferences.moods.length}개 무드 기반으로 선정되었습니다
          </p>
            ...
            {movies.map(movie => (
              ...
              >
                <div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-2 border-2 border-transparent group-hover:border-purple-500 transition-all">
                  ...
                  >
                    <Heart className={`w-3 h-3 transition-all ${favorites.includes(movie.id) ? 'fill-current text-red-500' : 'text-white'}`} />
                  </Button>
                ...
                <h4 className="text-white mb-1 truncate text-sm">{movie.title}</h4>
